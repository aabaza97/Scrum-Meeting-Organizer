...
class ScrumTimer: ObservableObject {
    ...
    @Published var secondsRemaining = 0 {
        didSet {
            guard secondsRemaining == .zero else { return }
            stopScrum()
        }
    }
     ...
     */
    init(lengthInMinutes: Int = 0, attendees: [ScrumPerson] = []) {
     ...
     */
    func reset(lengthInMinutes: Int, attendees: [ScrumPerson]) {
        self.lengthInMinutes = lengthInMinutes
        self.speakers = attendees.speakers
        secondsRemaining = lengthInSeconds
        activeSpeaker = speakerText
...
extension Array where Element == ScrumPerson {
